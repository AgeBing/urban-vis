...
| 参数项   | 格式                               | 示例                                                    |
| -------- | ---------------------------------- | --------------------------------------------------- |
| `ids`   数据 dataid 数组 | `string[]`|    |
| `source`  原始数据源类型（即上述data对应的数据源）  | `mobileTraj=0、taxiTraj=1` |  0  |
...
| `step` | 1、2、3、4 | 根据 Step 值插入某条数据，返回 |
...
[
    {
        "value": 0,  // value 值需要经过强化学习计算出区域概率
