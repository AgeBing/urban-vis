...
export const bbox = (points: Point[]):BBox => {
  ...
  points.forEach((p:Point) => {
    if (p.longitude > box[0]) { box[0] = p.longitude; }
    if (p.longitude < box[1]) { box[1] = p.longitude; }
    if (p.latitude > box[2]) { box[2] = p.latitude; }
    if (p.latitude < box[3]) { box[3] = p.latitude; }
  });
  return box;
};
...
export const timeExtend = (points: Point[]):TimeExtend => {
  const me: MomentExtend = [];
  if (points.length < 1) {
    throw Error('时间区间输入值错误！');
  }
  let t1 = moment(points[0].time),
    t2;
  me[0] = t1;
  me[1] = t1;
  ...
  points.forEach((p:Point) => {
    t2 = moment(p.time);
    if (t2.isBefore(me[0])) { me[0] = t2; }
    if (t2.isAfter(me[1])) { me[1] = t2; }
  });

  const extendTime: TimeExtend = [
